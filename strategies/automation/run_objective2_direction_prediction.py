...
from objective2_modeling import (
    build_direction_prediction_frame,
...
DEFAULT_OUT_DIR = REPO_ROOT / "outputs" / "objective2_direction_prediction"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Objective 2 next-direction prediction using the signal matrix.")
...
def main() -> None:
    ...
    y_selection = bundle.selection_df[bundle.target_direction_column].to_numpy(dtype=float)
    X_evaluation = bundle.evaluation_df[bundle.feature_columns].to_numpy(dtype=float)

    model = fit_logistic_regression(X_selection, y_selection)

    selection_probabilities = predict_logistic_probability(model, X_selection)
    evaluation_probabilities = predict_logistic_probability(model, X_evaluation)

    selection_pred_df = build_direction_prediction_frame(
        bundle.selection_df,
    ...
    evaluation_pred_df = build_direction_prediction_frame(
    ...
    selection_summary = summarize_direction_predictions(
        selection_pred_df,
        target_direction_column=bundle.target_direction_column,
    )
    evaluation_summary = summarize_direction_predictions(
        evaluation_pred_df,
        target_direction_column=bundle.target_direction_column,
    )

    selection_csv = maybe_tagged_path(out_dir, "selection_direction_predictions", ".csv", args.tag)
    evaluation_csv = maybe_tagged_path(out_dir, "evaluation_direction_predictions", ".csv", args.tag)
    summary_json = maybe_tagged_path(out_dir, "direction_prediction_summary", ".json", args.tag)
    ...
    print("=" * 80)
    print("OBJECTIVE 2 DIRECTION PREDICTION")
    print("=" * 80)
    ...
    print(
        "SELECTION: "
        f"Acc={selection_summary['accuracy']:.4f}, "
        f"LogLoss={selection_summary['log_loss']:.6f}, "
        f"LSReturn={selection_summary['long_short_strategy_return']:.6f}"
    )
    print(
        "EVALUATION: "
        f"Acc={evaluation_summary['accuracy']:.4f}, "
        f"LogLoss={evaluation_summary['log_loss']:.6f}, "
