...
def main() -> None:
    ...
    coarse_results = coarse_grid_search(A, step=args.coarse_step, top_k=args.top_k)
    if not coarse_results:
        raise RuntimeError("Coarse grid search found no feasible solutions.")
    ...
    numerical_best = solve_numerical_optimization(A, x0=local_best.weights)
    selection_optimized_combined_return = cumulative_return(A @ numerical_best.weights)
    ...
    evaluation_optimized_combined_return = cumulative_return(evaluation_A @ numerical_best.weights)
    ...
    results_to_frame(coarse_results).to_csv(coarse_path, index=False)
    results_to_frame(local_results or [local_best]).to_csv(local_path, index=False)
    ...
    summary = {
        ...
        "coarse_best": {
            "weights": coarse_best.weights.tolist(),
            "objective_value": coarse_best.objective_value,
        },
        "local_best": {
            "weights": local_best.weights.tolist(),
            "objective_value": local_best.objective_value,
        },
        "numerical_best": {
            "weights": numerical_best.weights.tolist(),
            "objective_value": numerical_best.objective_value,
        },
        "selection_summary": {
            "buy_and_hold_return": selection_buy_hold_return,
            "standalone_returns": selection_standalone_returns,
            "optimized_combined_return": selection_optimized_combined_return,
        },
        "evaluation_summary": {
            "buy_and_hold_return": evaluation_buy_hold_return,
            "standalone_returns": evaluation_standalone_returns,
            "optimized_combined_return": evaluation_optimized_combined_return,
        },
    }
    ...
    print(f"Optimized combined return: {selection_optimized_combined_return:.6f}")
    print("-" * 80)
    print(f"Coarse best weights:    {np.round(coarse_best.weights, 4)}")
    print(f"Coarse best objective:  {coarse_best.objective_value:.6f}")
    print(f"Local best weights:     {np.round(local_best.weights, 4)}")
    print(f"Local best objective:   {local_best.objective_value:.6f}")
    print(f"Numerical best weights: {np.round(numerical_best.weights, 4)}")
    print(f"Numerical best objective:{numerical_best.objective_value:.6f}")
    print("\n" + "=" * 80)
    ...
    print(f"Selection-weight combined return: {evaluation_optimized_combined_return:.6f}")
