...
DEFAULT_OUT_DIR = REPO_ROOT / "outputs" / "objective2_single_strategy_horizon_scan"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scan future return horizons using one strategy-score feature at a time."
    )
...
def main() -> None:
    ...
    for horizon_days in range(args.min_horizon_days, args.max_horizon_days + 1):
        ...
        for feature_column in bundle.feature_columns:
            X_selection = bundle.selection_df[[feature_column]].to_numpy(dtype=float)
            y_selection = bundle.selection_df[bundle.target_return_column].to_numpy(dtype=float)
            X_evaluation = bundle.evaluation_df[[feature_column]].to_numpy(dtype=float)
    ...
    summary_csv = maybe_tagged_path(out_dir, "single_strategy_horizon_scan_summary", ".csv", args.tag)
    summary_df.to_csv(summary_csv, index=False)

    print("=" * 80)
    print("OBJECTIVE 2 SINGLE-STRATEGY HORIZON SCAN")
    print("=" * 80)
    print(summary_df.head(20).to_string(index=False))
    print(f"[OK] Saved single-strategy horizon scan summary: {summary_csv}")
