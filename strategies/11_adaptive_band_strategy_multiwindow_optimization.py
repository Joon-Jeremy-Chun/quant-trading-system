...
OUT_DIR = Path("../outputs/11_adaptive_band_strategy_optimization")
FIGURES_ROOT = Path("../figures")
STRATEGY_NAME = "adaptive_band_strategy"
...
# Horizons ending at TRAIN_END_DATE
HORIZONS = {
    "1m": {"months": 1},
    "6m": {"months": 6},
    "1y": {"years": 1},
...
# MA windows will be filtered automatically inside each horizon
...
# Require enough rows after rolling
...
def optimize_horizon(df_all: pd.DataFrame, horizon_name: str, horizon_cfg: dict) -> None:
    ...
    for ma, uk, lk in product(valid_ma_windows, UPPER_KS, LOWER_KS):
        combo_idx += 1
        params = ParamSet(ma_window=int(ma), upper_k=float(uk), lower_k=float(lk))
