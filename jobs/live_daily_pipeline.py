...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the live multi-asset daily pipeline using existing signals by default."
    )
    parser.add_argument(
        "--symbol",
        type=str,
...
def legacy_main() -> None:
    ...
    if args.build_signal:
        pipeline_steps.append(
            run_step(
                ...
                [
                    py,
                    str(REPO_ROOT / "jobs" / "update_daily_price_data.py"),
                    "--max-staleness-days",
    ...
    pipeline_steps.append(
        run_step(
            "submit_gld_order",
            [py, str(REPO_ROOT / "jobs" / "legacy" / "legacy_tranche_order.py"),
    ...
    pipeline_steps.append(
        run_step(
            "send_email_alert",
            [py, str(REPO_ROOT / "jobs" / "send_live_daily_report.py")],
            REPO_ROOT,
        )
    )
    ...
    if args.skip_orders:
        ...
        pipeline_steps.append(
            run_step(
                ...
                [py, str(REPO_ROOT / "jobs" / "execute_delta_tranche_orders.py")],
                REPO_ROOT,
            )
        )
