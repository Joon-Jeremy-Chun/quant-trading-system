...
\ldots, 80\}$ reveals a non-monotone pattern for GLD h130: the baseline
$N = 10$ achieves the highest Sharpe (1.08), which drops sharply at $N = 20$
(0.94) before partially recovering to 1.01--1.05 in the $N = 30$--70 range.
This pattern suggests that the top-10 strategies per family already capture
the most predictive market dynamics; additional strategies introduce correlated
noise that Lasso can partially filter but not fully overcome. The $N = 10$
configuration is thus near-optimal for GLD and is used as the primary setting
throughout this work. For the short-horizon alternative (h45), Sharpe remains
consistently low across all $N$ (0.17--0.49), confirming that the horizon
alignment is the dominant driver of performance rather than feature count
(Figure~\ref{fig:topn}).
