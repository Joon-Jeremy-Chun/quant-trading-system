...
(ML-guided sparse prediction). Applied to the SPDR Gold ETF (GLD) and
Berkshire Hathaway Class B (BRK-B) over a five-year forward test
(2020--2024), Stage C achieves a Sharpe ratio of 1.08 for GLD and 2.16 for
BRK-B, outperforming Stage A and B in every evaluation year. A key finding is
that prediction horizon alignment---matching the model's target horizon to the
evaluation window used in Layer 1---is critical for out-of-sample
generalization.
\end{abstract}
...
from the current one \citep{lopezdeprado2018}.
...
using three assets (GLD, BRK-B) and deployed in a live paper-trading
environment on a Raspberry Pi, generating daily signals and executing
fractional-share tranche orders via the Alpaca API.

% =============================================================================
\section{Background}
% =============================================================================

\subsection{Financial Prediction and the Limits of Standard ML}

The efficient market hypothesis suggests that asset prices reflect available
information, making systematic prediction difficult in theory
\citep{fama1970}. In practice, a growing body of work demonstrates that
short-horizon predictability exists, particularly in the form of momentum
\citep{jegadeesh1993} and mean-reversion signals conditioned on volatility
\citep{bollinger2002}. However, realizing this predictability in a live system
requires models that remain robust when trained on limited data.

\citet{gu2020} demonstrate that machine learning models substantially
outperform traditional linear factor models in cross-sectional return
prediction when given access to hundreds of characteristic-based features.
Their findings highlight the importance of regularization and feature
selection in high-dimensional financial settings. The present work differs in
that it focuses on a single asset with a time-series prediction task, and
explicitly constructs its feature space from interpretable trading signals.

\subsection{Sparse Linear Regression}

Ridge regression \citep{hoerl1970} imposes an $\ell_2$ penalty on the
coefficient vector, shrinking all coefficients toward zero without eliminating
any. The Lasso \citep{tibshirani1996} uses an $\ell_1$ penalty, which drives
many coefficients to exactly zero and thus performs implicit feature selection.
The Elastic Net \citep{zou2005} combines both penalties, providing a
compromise that handles correlated predictors better than the Lasso alone.
All three are used in Layer 2 of the present framework, with the active model
selected monthly by cross-validated mean squared error.

\subsection{Rolling Validation in Finance}

Standard $k$-fold cross-validation is inappropriate for time-series data due
to look-ahead bias. \citet{lopezdeprado2018} advocates walk-forward validation
with embargo periods to prevent data leakage. The present work implements a
strict rolling forward test: model parameters are estimated on data available
at each anchor date, and performance is measured on the subsequent hold period
with no overlap between training and evaluation windows.
...
of the design matrix $A$ and represents a trade-off between expressiveness and
overfitting. The cross-validated MSE curve across $N \in \{10, 20, 30,
\ldots, 80\}$ follows an inverted-U shape for GLD: MSE improves as $N$
increases from 10 to approximately 50--70, then degrades as additional
strategies add noise that Lasso cannot fully filter. Sharpe ratio exhibits the
same qualitative pattern, with a recommended operating range of $N = 50$--70
for GLD (Sharpe 1.03--1.05 vs 1.08 at $N = 10$; the small gap suggests the
$N = 10$ baseline is already near-optimal for GLD).

\subsection{BRK-B Results}

The same pipeline was applied to Berkshire Hathaway Class B (BRK-B) using 79
anchor snapshots spanning the same 2019--2026 period. The five-year forward
test (2020--2024) yields a capital efficiency Sharpe of 2.16 with positive
returns in every calendar year, including the 2022 market downturn ($+$8.4\%
capital efficiency). The five-year cumulative strategy return is $+$70.9\%
versus B\&H $+$98.5\%, but at an average exposure of only 43\%, indicating
substantial idle capital available for simultaneous deployment in a
multi-asset portfolio.

\subsection{Live Deployment}

The full pipeline is deployed on a Raspberry Pi 4 running a systemd timer
that triggers daily at 13:00 PDT on trading days. As of April 2026, the
current GLD signal is BUY with weight 1.00 under a Ridge model with anchor
2026-03-31, predicting $+$55.6\% over the next 130 days. The BRK-B signal is
BUY with weight 0.55 under an Elastic Net model. Both signals are generated
from data fetched via the Alpaca Markets API and committed to version control
automatically.
...
structured signals---analogous to the role of hand-crafted convolutional
filters in early computer vision architectures \citep{lecun1989}. This
transformation is not learned from data, which means it does not consume
sample budget. Layer 2 then operates in a space of $N_{\text{total}}$
...
on two assets over a single five-year period; out-of-sample generalization to
...
which would likely reduce absolute returns, particularly for high-frequency
rebalancing.

Future work will extend the framework to a fully simultaneous multi-asset
allocation, where the idle capital of one asset can be deployed into another
based on uncorrelated signals. An oracle simulation conducted over 57 asset
combinations (6 assets taken 2--6 at a time) establishes that GLD $+$ BRK-B
$+$ QQQ provides the highest risk-adjusted oracle bound (Sharpe 3.02 under
proportional allocation), motivating this as the next experimental target.
Additional planned extensions include widening the Layer 1 evaluation horizons
to 18 and 24 months, and activating live execution by setting
\texttt{ALPACA\_DRY\_RUN=false}.
...
contribute measurably to out-of-sample performance. Stage C achieves a
five-year Sharpe ratio of 1.08 on GLD and 2.16 on BRK-B, outperforming the
buy-and-hold benchmark on a risk-adjusted, capital-efficiency basis in both
cases. The system is live, automated, and extensible to a multi-asset
...
\begin{tikzpicture}[xscale=1.0, yscale=1.7]
  ...
  \draw[qNavy, thick] plot[mark=*, mark size=2pt] coordinates {
    (1,1.08)(2,1.04)(3,1.05)(4,1.03)(5,1.05)(6,1.02)(7,1.01)(8,0.99)
  };

  % h45 line
  \draw[red!70, thick, dashed] plot[mark=square*, mark size=2pt] coordinates {
    (1,0.33)(2,0.35)(3,0.34)(4,0.36)(5,0.35)(6,0.34)(7,0.33)(8,0.31)
  };
...
(GLD, 2020--2024).} The h130 Sharpe peaks near $N = 10$--50 and degrades
gradually beyond $N = 60$. The h45 Sharpe remains consistently low across
all $N$, confirming that the horizon alignment is the primary driver of
performance rather than the number of features.}
\label{fig:topn}
\end{figure}
