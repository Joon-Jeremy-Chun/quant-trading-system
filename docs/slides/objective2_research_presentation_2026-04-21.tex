...
\begin{frame}{Outline}
  \begin{enumerate}
    ...
    \item \textbf{Two-Layer Architecture} \hfill \small\textit{slide 16}\\[-0.1em]
      \footnotesize Nonlinear basis (Layer 1) + sparse linear combiner (Layer 2)
    \normalsize\vspace{0.3em}

    \item \textbf{Stage C: ML-Guided} \hfill \small\textit{slides 17--21}\\[-0.1em]
      \footnotesize 40-column design matrix, supervised learning, continuous exposure
    \normalsize\vspace{0.3em}

    \item \textbf{Results \& Analysis} \hfill \small\textit{slides 22--25}\\[-0.1em]
      \footnotesize Three-stage comparison, update frequency, hold horizon
    \normalsize\vspace{0.3em}

    \item \textbf{Live Signal Example} \hfill \small\textit{slide 26}\\[-0.1em]
      \footnotesize Stage C running in production on Raspberry Pi
    \normalsize\vspace{0.3em}

    \item \textbf{Next Steps \& Conclusion} \hfill \small\textit{slides 27--28}
  \end{enumerate}
\end{frame}
...
% ── Slide NEW: Two-Layer Mathematical Architecture ───────────────────────────
\begin{frame}{Why This Works: A Two-Layer Mathematical Structure}
  \textbf{The challenge in applying ML to financial time series:}
  \begin{itemize}
    \item More features $\Rightarrow$ need more data to avoid overfitting
    \item Longer history $\Rightarrow$ captures outdated regimes, misses trend shifts
  \end{itemize}

  \vspace{0.5em}
  \textbf{Our solution: structured features + sparse linear combiner}

  \vspace{0.4em}
  \begin{center}
  \begin{tikzpicture}[
    node distance=0.6cm and 1.2cm,
    box/.style={draw, rounded corners, fill=qLight, minimum width=2.4cm, minimum height=0.7cm,
                align=center, font=\small},
    arrow/.style={-{Latex}, thick, color=qNavy},
  ]
    \node[box, fill=qGold!30] (price) {Price data\\$p(t)$};

    \node[box, right=of price, fill=qMid!20, minimum width=3.2cm] (layer1)
      {Layer 1: Nonlinear\\$\varphi_i(p) \to s_i(t)$};

    \node[box, right=of layer1, fill=qLight, minimum width=2.8cm] (matA)
      {Strategy matrix\\$A \in \mathbb{R}^{T \times N}$};

    \node[box, right=of matA, fill=qMid!20, minimum width=3.0cm] (layer2)
      {Layer 2: Linear\\$\hat{y} = A\boldsymbol{\beta}$};
    ...
    \node[below=0.25cm of layer1, font=\footnotesize, color=qNavy]
      {MA, bands, vol — \emph{domain knowledge}};
    \node[below=0.25cm of layer2, font=\footnotesize, color=qNavy]
      {Ridge/Lasso — \emph{sparse, interpretable}};
  \end{tikzpicture}
  \end{center}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Layer 1 (fixed, nonlinear)}
      \begin{itemize}\small
        \item Converts prices into structured signals
        \item Not arbitrary — grounded in trading theory
        \item Same as a fixed first layer of a neural net
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Layer 2 (learned, linear)}
      \begin{itemize}\small
        \item Trained on a 1-year rolling window
        \item Lasso drives most $\beta_i \to 0$ (automatic feature selection)
        \item Updated monthly $\Rightarrow$ adapts to regime changes
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Hold Horizon: 45 Days vs 130 Days}
  \small
  Stage C $\cdot$ 1-month update $\cdot$ top-10 $\cdot$ Ret / Exposure / Efficiency
  \quad {\footnotesize (both models trained and invested at same horizon)}

  \vspace{0.4em}
  \begin{tabular}{lrcc}
    \toprule
    Year & B\&H & \textbf{h130} (train 130d, hold 130d) & h45 (train 45d, hold 45d) \\
    \midrule
    2020 & $+$23.9\% & \textbf{$+$10.9\%} / 57\% / $+$19\% & $+$11.1\% / 60\% / $+$19\% \\
    2021 & $-$6.2\%  & \textbf{$+$0.8\%} / 29\% / $+$3\%  & $-$9.9\% / 54\% / \textcolor{red}{$-$18\%} \\
    2022 & $+$0.8\%  & \textbf{$-$0.2\%} / 31\% / $-$1\%  & $-$1.6\% / 35\% / $-$5\% \\
    2023 & $+$11.8\% & \textbf{$+$4.0\%} / 32\% / $+$13\% & $+$4.3\% / 74\% / $+$6\% \\
    2024 & $+$27.0\% & $+$16.4\% / 65\% / $+$25\% & \textbf{$+$24.1\%} / 83\% / $+$29\% \\
    \midrule
    Sharpe & --- & \textbf{1.08} & 0.33 \\
    \bottomrule
  \end{tabular}

  \vspace{0.4em}
  \begin{itemize}
    \item \textbf{h130 dominates}: 2021 bear year — h130 $+$3\% eff vs h45 \textcolor{red}{$-$18\% eff}
    \item \textbf{Why h130?} \ Stage A/B optimize strategies over a \textbf{6-month} evaluation window
      ($\approx$130 trading days) $\Rightarrow$ feature--target alignment in Layer 2
    \item h45 model sees noisier 45-day targets $\Rightarrow$ high variance, poor generalization
  \end{itemize}
\end{frame}
...
\begin{frame}{What's Next? — Priorities}
  ...
  \begin{itemize}
    ...
    \item Practical first step: quadratic terms only (\(s_i^2\), 40 extra columns) ---
          cross-terms (\(s_i s_j\)) create 780 columns, too high-dimensional for a 1-year window
  \end{itemize}

  \vspace{0.3em}
  \begin{block}{Remark: Stage C is already a 2-layer nonlinear system}
    \small
    The strategy score functions are themselves \emph{nonlinear} transformations of price data
    (moving averages, band crossings, volatility regimes).
    Stacking a linear model on top of these features is equivalent to a shallow neural network
    with a fixed nonlinear first layer.
    This may explain why simple Ridge/Lasso already produces strong forward results ---
    the expressiveness is in the features, not the final combiner.
  \end{block}
