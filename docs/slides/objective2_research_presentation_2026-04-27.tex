...
% ── Slide 5: The Data ─────────────────────────────────────────────────────────
\begin{frame}{The Data and Strategy Space}

  \textbf{Asset:} GLD (SPDR Gold Shares ETF) --- daily OHLCV, 2015--2026 ($\approx$2,750 rows)

  \vspace{0.5em}
  \textbf{Layer 1 produces 4 strategy families:}

  \vspace{0.3em}
  \begin{tabular}{llp{5.5cm}}
    \toprule
    Family & Signal & Key parameter \\
    \midrule
    Adaptive Band       & Band crossover vs.\ rolling mean  & window, upper/lower $k$ \\
    MA Crossover        & Short MA crosses long MA           & short window, long window \\
    Adaptive Vol.\ Band & Volatility-scaled band             & window, vol multiplier \\
    Fear-Greed C\&V     & Candle pattern + volume spike      & candle size, volume threshold \\
    \bottomrule
  \end{tabular}

  \vspace{0.5em}
  Each family is grid-searched over its parameters on a \textbf{1-year selection window}.
  Top-$N$ results per family feed into matrix $A$ --- giving \textbf{40 columns} for $N=10$.

  \vspace{0.4em}
  \begin{block}{Key design choice}
    These features are \emph{not arbitrary}: each column is a strategy that would be
    traded by a practitioner.  The mathematical structure of Layer 1 constrains
    the feature space before any learning occurs.
  \end{block}
...
\begin{frame}{Building Complexity in Three Stages}
  ...
  \begin{tabular}{p{1.5cm}p{4.2cm}p{4.2cm}p{3.0cm}}
    \toprule
    & \textbf{Stage A} & \textbf{Stage B} & \textbf{Stage C} \\
    \midrule
    \textbf{Question}
      & Which single strategy is best right now?
      & What weighted combination maximizes historical return?
      & Which combination best \emph{predicts future} return? \\[0.4em]
    \textbf{Method}
      & Grid search, pick rank-1
      & Constrained optimization over $A\mathbf{x}$
      & Supervised ML on $A$: Ridge, Lasso, ElasticNet \\[0.4em]
    \textbf{Limitation}
      & Partial signal only
      & Fixed weights; no future prediction
      & (none --- this is Stage C) \\
    \bottomrule
  \end{tabular}

  \vspace{0.5em}
  Each stage builds on the previous.
  Stage C is \textbf{the same matrix $A$} but instead of fitting weights to past returns,
  we fit a model to \emph{predict} the next 130-day return.
...
\begin{frame}{Results: Stages A, B, and C Compared}
  ...
  \small
  Hold 130 days $\cdot$ 1-month update $\cdot$ GLD 2020--2024 forward test

  \vspace{0.4em}
  \begin{tabular}{lrrrrr}
    \toprule
    Year & B\&H & \textbf{A}: Single Signal & \textbf{B}: Ensemble & \textbf{C}: ML-Guided \\
    \midrule
...
\begin{frame}{Appendix D: Strategy Parameter Ranges (Grid Search)}
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    Adaptive Band       & MA window & 5, 10, 22, 65, 130, 260 days \\
                        & Upper $k$ & $-3.0$ to $+3.0$ (step 0.1) \\
                        & Lower $k$ & $-3.0$ to $+3.0$ (step 0.1) \\
    \midrule
    MA Crossover        & Short MA  & 5 to 95 days (step 5) \\
                        & Long MA   & short$+$5 to 140 days \\
    \midrule
    Adaptive Vol.\ Band & MA window & 5, 10, 22, 65, 130, 260 days \\
                        & Vol multiplier & $0.5$ to $3.0$ (step 0.1) \\
    \midrule
    Fear-Greed C\&V     & Candle threshold & $0.3\sigma$ to $2.0\sigma$ \\
                        & Volume threshold & $1.0\times$ to $3.0\times$ avg \\
    \bottomrule
  \end{tabular}

  \vspace{0.4em}
  Grid search runs in parallel using \texttt{joblib} (4.4$\times$ speedup on 20-core CPU).
  Top-$N$ results per family are selected by total return in the selection window.
\end{frame}
