...
\begin{frame}{Outline}
  \begin{enumerate}
    \item \textbf{The Question} \hfill \textit{slide 3}\\[-0.2em]
      {\small What are we predicting, and why is it a well-defined problem?}

    \vspace{0.3em}
    \item \textbf{The Challenge} \hfill \textit{slide 4}\\[-0.2em]
      {\small Why standard ML fails on financial time series}

    \vspace{0.3em}
    \item \textbf{Our Solution: Two-Layer Architecture} \hfill \textit{slides 5--7}\\[-0.2em]
      {\small Strategy vector space (Layer 1) + sparse linear model (Layer 2)}

    \vspace{0.3em}
    \item \textbf{Validation Design} \hfill \textit{slide 8}\\[-0.2em]
      {\small Rolling forward test — no look-ahead bias}

    \vspace{0.3em}
    \item \textbf{Results} \hfill \textit{slides 9--10}\\[-0.2em]
      {\small Stage A vs B vs C comparison; why 130 days works}

    \vspace{0.3em}
    \item \textbf{Live System \& Conclusion} \hfill \textit{slides 11--12}\\[-0.2em]
      {\small Running in production; back to the question}
  \end{enumerate}
\end{frame}
...
\begin{tabular}{p{1.4cm}p{3.6cm}p{3.6cm}p{3.6cm}}
  ...
  \textbf{Method}
    & Grid search,\newline pick rank-1
    & Constrained opt.\newline over $A\mathbf{x}$
    & Ridge / Lasso /\newline ElasticNet on $A$ \\[0.3em]
  \textbf{Limit}
    & Partial signal
    & Fixed weights,\newline no prediction
    & --- \\
  \bottomrule
\end{tabular}
...
it fits a model to \emph{predict} the next 130-day return.\\[0.2em]
{\footnotesize $^*$Sharpe 1.12 with $N=20$ after label-leakage correction (strict cutoff).}
...
\begin{frame}{Results: Stages A, B, and C Compared}
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    \textbf{Sharpe (eff.)} & --- & $\approx$0.5 & $\approx$0.7 & \textbf{1.12}$^*$ \\
    \bottomrule
  \end{tabular}
