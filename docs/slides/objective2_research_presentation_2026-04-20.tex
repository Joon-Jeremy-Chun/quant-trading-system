...
\begin{frame}{Reading the Numbers: Capital Efficiency}
  \textbf{Why does our strategy return less than Buy-and-Hold?}

  \vspace{0.6em}
  Buy-and-hold is \textbf{always 100\% exposed}. Our strategy sits out when
  the signal is weak — gross exposure typically ranges from 21\% to 78\%.

  \vspace{0.6em}
  A fairer comparison: \textbf{return per unit of capital deployed}.
  \[
    \text{Efficiency} = \frac{\text{Return}}{\text{Avg. Exposure}}
  \]

  \vspace{0.5em}
  \begin{tabular}{lrrr}
    \toprule
    2024 & Return & Avg.\ Exposure & Efficiency \\
    \midrule
  ...
  \vspace{0.6em}
  \begin{itemize}
    \item Hold-45 Stage C \textbf{exceeds} B\&H efficiency in 2024
    \item The idle capital (22--79\%) can be deployed into other assets
    \item This is the foundation of a multi-asset allocation framework
  \end{itemize}
\end{frame}
...
\begin{frame}{How Often Should the Model Update?}
  \small
  \textbf{Hold 130 days} \hfill Ret / Avg.\ Exposure
  ...
  \vspace{0.5em}
  \textbf{Hold 45 days} \hfill Ret / Avg.\ Exposure
  ...
  \vspace{0.4em}
  \begin{itemize}
    \item \textbf{1M consistently best} across both horizons: fastest adaptation captures regime shifts
    \item Longer intervals miss the 2024 bull run (h130 6M: +5.5\% vs 1M: +16.4\%)
  \end{itemize}
\end{frame}

% ── NEW: Hold Horizon Comparison ─────────────────────────────────────────────
\begin{frame}{Hold Horizon: 45 Days vs 130 Days}
  \small
  Stage C $\cdot$ 1-month update $\cdot$ \textbf{Ret / Exposure / Efficiency}
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    2020 & +23.9\% / 100\% / +24\% & +10.9\% / 57\% / +19\% & \textbf{+17.4\%} / 72\% / \textbf{+24\%} \\
    2021 & $-$6.2\% / 100\% / $-$6\% & \textbf{+0.8\%} / 29\% / +3\% & +1.5\% / 35\% / \textbf{+4\%} \\
    2022 & +0.8\% / 100\% / +1\% & \textbf{$-$0.2\%} / 31\% / $-$1\% & $-$0.6\% / 37\% / $-$2\% \\
    2023 & +11.8\% / 100\% / +12\% & \textbf{+4.0\%} / 32\% / +12\% & +2.5\% / 44\% / +6\% \\
    2024 & +27.0\% / 100\% / +27\% & +16.4\% / 65\% / +25\% & \textbf{+25.2\%} / 78\% / \textbf{+32\%} \\
    \bottomrule
  \end{tabular}

  \vspace{0.6em}
  \begin{itemize}
    \item \textbf{h45 captures bull markets better} (2020: +17.4\%, 2024: +25.2\%) — faster turnover
    \item \textbf{h130 more stable in flat/bear markets} (2022: $-$0.2\% vs $-$0.6\%, 2023: +4.0\% vs +2.5\%)
    \item \textbf{h45 efficiency in 2024 (+32\%) exceeds B\&H (+27\%)} — same capital, better return per unit deployed
    \item Hold horizon is a hyperparameter: h45 for bull-market capture, h130 for stability
  \end{itemize}
\end{frame}
...
\begin{frame}{What's Next? — Priorities}
  ...
  \begin{itemize}
    \item Apply the same framework to BRK.B, oil, copper, equities
    \item When GLD exposure is low, deploy idle capital into other assets
  ...
  \begin{itemize}
    \item Expand the library with nonlinear terms: \(\Theta(A) = [s_i,\; s_i^2,\; s_i s_j,\; \dots]\)
  ...
  \begin{itemize}
    \item Layer 1: direction signals $\;\longrightarrow\;$ Layer 2: timing signals
...
\begin{frame}{Conclusion}
  ...
  \vspace{0.5em}
  \textbf{Live:} Running on Raspberry Pi + Alpaca paper trading since April 2026.

  \vspace{0.3em}
  \textbf{Next:}
  \begin{itemize}
    \item Multi-asset expansion (BRK.B, oil, copper) — data already collected
    \item Regime-aware exposure control layer
    \item Nonlinear model selection (SINDy)
  \end{itemize}
\end{frame}
